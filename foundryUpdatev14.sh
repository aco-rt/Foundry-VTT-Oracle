...
pm2 stop all
# Remove the old start up path that was used in v13 and below, let's remove the process
pm2 delete foundry
# Remove the current pm2 from startup to allow for the upgrade.
pm2 unstartup
# Add the new method of installing NodeJS repository and update the installed version of NodeJS to v24.
...
# Let's update foundry to v14 here
# stopping foundry
pm2 stop foundry
# making a copy so we can go back if needed.
mv foundry foundry-archive-v13
# Getting and installing foundry
mkdir ~/foundry
# Enter Foundry timed url for download
echo "please enter the foundry vtt v14 timed download url for the Linux NodeJS version"
read tdurl
